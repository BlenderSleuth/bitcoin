...
###   schleemscore.org and schleems.org and compares them.
...
WORKINGDIR="/tmp/schleems_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://schleemscore.org"
HOST2="https://schleems.org"
BASEDIR="/bin/"
VERSIONPREFIX="schleems-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'schleems-',
...
if [ $? -ne 0 ]; then
   echo "schleems.org failed to provide signature file, but schleemscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "schleems.org and schleemscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Schleems Core binary release signing key installed?"
   fi
